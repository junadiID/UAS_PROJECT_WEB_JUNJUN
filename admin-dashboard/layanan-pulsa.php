...
require '../lib/session_login_admin.php'; 
        ...
        if (isset($_POST['tambah'])) {
            ...
            if (!$sid || !$pid || !$operator || !$layanan || !$harga || !$harga_api || !$profit || !$status || !$provider) {
                ...
                if ($conn->query("INSERT INTO layanan_pulsa VALUES ('', '$sid', '$pid', '$layanan', '$operator', '$harga', '$harga_api', '$profit', '$status', '$provider')") == true) {
            ...
            if ($cek_id->num_rows == 0) {
                ...
                if ($conn->query("UPDATE layanan_pulsa SET service_id = '$sid', provider_id = '$pid', layanan = '$layanan', operator = '$operator', harga = '$harga', harga_api = '$harga_api', profit = '$profit', status = '$status', provider_id = '$pid', provider = '$provider' WHERE service_id = '$sid'") == true) {
                    $_SESSION['hasil'] = array(
                        'alert' => 'success', 
                        'judul' => 'Berhasil', 
                        'pesan' => '
            ...
            <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                    <button data-toggle="modal" data-target="#addModal" class="btn btn-success btn-bordred waves-effect waves-light m-b-30"><i class="fa fa-plus"></i> Tambah Layanan</button> 
                                <h4 class="m-t-0 header-title"><b><i class="fa fa-list"></i>    Layanan Sosial Media</b></h4>                             
                                        <form>
                                            <div class="row">
...
?>
                                    <tr> 
                                        ...
                                        <td align="center">
                                            <a href="javascript:;" onclick="users('<?php echo $config['web']['url']; ?>admin-dashboard/ajax/layanan-pulsa/edit?id_layanan=<?php echo $data_layanan['service_id']; ?>')" class="btn btn-sm btn-warning"><i class="fa fa-pencil" title="Edit"></i></a>
                                            <a href="javascript:;" onclick="users('<?php echo $config['web']['url']; ?>admin-dashboard/ajax/layanan-pulsa/delete?id_layanan=<?php echo $data_layanan['service_id']; ?>')" class="btn btn-sm btn-danger"><i class="fa fa-trash" title="Hapus"></i></a>
                                        </td>                                 
                                    </tr>  
<?php } ?>
                                    </tbody>
