...
$cek_pesanan = $conn->query("SELECT * FROM pembelian_sosmed WHERE status IN ('','Pending','Processing') AND provider = 'MAUPEDIA'");
...
if (mysqli_num_rows($cek_pesanan) == 0) {
  ...
  if ($o_provider == "MANUAL") {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://maupedia.com/api/status-sosmed");
    ...
    if ($update_pesanan == TRUE) {
      echo "".$data_pesanan['provider_oid']." status $u_status | start $u_start | remains $u_remains<br />";
    } else {
      echo "Error database.";
    }
  }
