		...
		if (mysqli_num_rows($cek_usernya) == 1) {
			if ($aksinya == 'pemesanan') {
					...
					if (!$layanan || !$target || !$jumlah) {
						...
						if (mysqli_num_rows($cek_layanan) == 0) {
							...
							if ($jumlah > $data_layanan['min']) {
								$hasilnya = array('status' => false, 'data' => array('pesan' =>'Jumlah Pemesanan Tidak Sesuai'));
								...
								if ($provider == "MANUAL") {
									...
									$ch = curl_init();
									curl_setopt($ch, CURLOPT_URL, "https://www.wstore.co.id/api/order");
								...
								if ($provider == "WSTORE" AND $resultnya['status'] == false) {
									$hasilnya = array('status' => false, 'data' => array('pesan' => 'Layanan Sedang Ada Gangguan'));
								} else {
									if ($provider == "WSTORE") {
				...
				if (isset($_POST['id'])) {
					...
					$cek_layanan = $conn->query("SELECT * FROM layanan_sosmed WHERE status = 'Aktif' AND tipe = 'SOSMED' ORDER BY service_id ASC");
