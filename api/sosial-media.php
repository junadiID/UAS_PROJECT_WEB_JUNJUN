		...
		if (mysqli_num_rows($cek_usernya) == 1) {
			if ($aksinya == 'pemesanan') {
						...
						if (mysqli_num_rows($cek_layanan) == 0) {
							...
							if ($jumlah > $data_layanan['min']) {
								...
								if ($provider == "MANUAL") {
									...
									$ch = curl_init();
									curl_setopt($ch, CURLOPT_URL, "https://maupedia.com/api/sosial-media-1");
								...
								if ($provider == "MEDANPEDIA" AND $resultnya['status'] == false) {
									$hasilnya = array('status' => false, 'data' => array('pesan' => ''.$resultnya['data']));
								} else {
				...
				if (isset($_POST['id'])) {
					...
					$cek_layanan = $conn->query("SELECT * FROM layanan_sosmed WHERE status = 'Aktif' AND tipe = 'Sosial Media' ORDER BY service_id ASC");
